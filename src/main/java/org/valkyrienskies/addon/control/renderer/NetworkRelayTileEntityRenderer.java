...
public class NetworkRelayTileEntityRenderer extends TileEntitySpecialRenderer<TileEntityNetworkRelay> {
    ...
    public void render(TileEntityNetworkRelay te, double x, double y, double z,
        ...
        VSNode thisNode = te.getBehaviour(NodeTEBehaviour.class).getNode();
        ...
        for (VSNode otherNode : thisNode.dfsIterable()) {
            WireType wireType = thisNode.edgeTo(otherNode);
            TileEntity otherTile = otherNode.getOwner();
            // Don't render the same connection twice.
            if (otherTile instanceof TileEntityNetworkRelay && otherTile.getPos().equals(te.getPos())) {
                continue;
            }

            if (wireType == WireType.VANISHING) {
            ...
            double endX = (startX * 2) - otherTile.getPos().getX();
            double endY = (startY * 2) - otherTile.getPos().getY() - 1.5;
            double endZ = (startZ * 2) - otherTile.getPos().getZ();

            int wt = wireType.ordinal();
            renderWire(x, y, z, startX, startY, startZ, endX, endY, endZ,
                colours[wt][0], colours[wt][1], colours[wt][2], colours[wt][3]);
    ...
    private void renderWire(double x, double y, double z,
        ...
        for (int j = 0; j <= 24; ++j) {
            ...
            bufferbuilder.pos(x + d13 * f3 + 0.0D,
                ...
                z + d15 * f3).color(r, g, b, a).endVertex();
            bufferbuilder
                .pos(x + d13 * f3 + 0.025D,
        ...
        for (int k = 0; k <= 24; ++k) {
            ...
            bufferbuilder.pos(x + d13 * f7 + 0.025D,
                ...
                z + d15 * f7 + 0.025D).color(r, g, b, a).endVertex();
        }
