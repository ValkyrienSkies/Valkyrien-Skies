...
public class ShipUUIDToPosData extends WorldSavedData {
	...
	public class ShipPositionData {
		...
		private ShipPositionData(PhysicsWrapperEntity wrapper) {
			...
			lToWTransform = RotationMatrices.convertToFloat(wrapper.wrapping.coordTransform.getCurrentTransform().getInternalMatrix(TransformType.LOCAL_TO_GLOBAL));
		}
		...
		private void updateData(PhysicsWrapperEntity wrapper) {
			...
			RotationMatrices.convertToFloat(wrapper.wrapping.coordTransform.getCurrentTransform().getInternalMatrix(TransformType.LOCAL_TO_GLOBAL), lToWTransform);
		}
