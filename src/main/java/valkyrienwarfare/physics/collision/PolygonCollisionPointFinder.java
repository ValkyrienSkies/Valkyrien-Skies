...
public class PolygonCollisionPointFinder {
	...
	public static Vector[] getPointsOfCollisionForPolygons(PhysPolygonCollider collider,
		...
		for (int cont = 0; cont < collider.collisions.length; cont++) {
			toCollideWith = collider.collisions[cont];
		...
		Vector contactPoint = collider.collisions[minCollisionIndex].movable.getVertices()[minCollisionIndexIndex];// contactPoint.getAddition(axis.getProduct(reverseVelocityAlongAxis
																													// *
																													// secondsAgo));

		return new Vector[] { contactPoint };
		// toCollideWith.firstContactPoint, toCollideWith.getSecondContactPoint() };
	}
