...
public class BigBastardMath{
	...
	/**
	 * Used to get a semi-random distribution of numbers. These numbers are then applied
	 * to stabilize the physics simulation.
	 * @param relativeTo
...
/*
	public static Quaternion bezierThoseQuats(double t,Quaternion... quats){
		double qX = bezierThatBastard(quats[0].x,quats[1].x,quats[2].x,quats[3].x,t);
		double qY = bezierThatBastard(quats[0].y,quats[1].y,quats[2].y,quats[3].y,t);
		double qZ = bezierThatBastard(quats[0].z,quats[1].z,quats[2].z,quats[3].z,t);
		double qW = bezierThatBastard(quats[0].w,quats[1].w,quats[2].w,quats[3].w,t);
		double length = Math.sqrt(qX*qX+qY*qY+qZ*qZ+qW*qW);
		qX/=length;qY/=length;qZ/=length;qW/=length;
		return new Quaternion(qX,qY,qZ,qW);
	}

	public static double[] bezierThoseAngles(double t,Quaternion... quats){
		double[] angles0 = quats[0].toRadians();
		double[] angles1 = quats[1].toRadians();
		double[] angles2 = quats[2].toRadians();
		double[] angles3 = quats[3].toRadians();
		double[] angles = new double[3];
		angles[0] = bezierThatBastard(angles0[0],angles1[0],angles2[0],angles3[0],t);
		angles[1] = bezierThatBastard(angles0[1],angles1[1],angles2[1],angles3[1],t);
		angles[2] = bezierThatBastard(angles0[2],angles1[2],angles2[2],angles3[2],t);
		return angles;
	}
*/
	/**
	 * Prevents sliding when moving on small angles dictated by the tolerance set in
	 * the ValkyrianWarfareMod class
