...
public class ConcurrentUpdatableIndexedCollection<O> implements IndexedCollection<O> {
	...
	public void updateAll(Collection<? extends O> c, Attribute<O, ?> attribute) {
	...
	public void update(O object, Attribute<O, ?> attribute) {
	...
	public void addIndex(Index<O> index, QueryOptions queryOptions) {
		if (!(index instanceof UpdatableIndex)) throw new IllegalArgumentException("Must be updatable index");
		queryOptions = openRequestScopeResourcesIfNecessary(queryOptions);
		try {
	...
	public CloseableIterator<O> iterator() {
		return new CloseableIterator<O>() {
			...
			public void remove() {
				...
				if (autoClosed) {
					ConcurrentUpdatableIndexedCollection.this.remove(currentObject); // reopens resources temporarily
				}
				else {
